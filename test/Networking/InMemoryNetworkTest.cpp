...
TEST(InMemoryNetwork_FullTest)
{
    ...
    network.add_host_to_network(address_1);
    network.add_host_to_network(address_2);

    network.create_listening_socket(address_1, port_1);
    auto data_socket_client = network.create_data_socket(address_2, address_1, port_2, port_1).value();
}
